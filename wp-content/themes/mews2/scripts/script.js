document.addEventListener("DOMContentLoaded", function(){
  ...
  const priceDescriptionContainersMobile = Array.from(document.querySelectorAll('.price__description-container-mobile'));
  ...
  const updateActivePrice = (activePriceIndex = 0) => {
    ...
    priceDescriptionContainersMobile.forEach((elem, index) => {
      if (elem.classList.contains('price__description-container_active')) {
        elem.classList.remove('price__description-container_active');
      }
    })
    ...
    priceDescriptionContainersMobile[activePriceIndex].classList.add('price__description-container_active');
  ...
  if (priceLinks.length > 0) {
    updateActivePrice(0); // Устанавливаем первый активный элемент
  }
});
